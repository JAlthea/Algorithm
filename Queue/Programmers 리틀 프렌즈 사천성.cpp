...
string solution(int m, int n, vector<string> board) {
    ...
    for (int i = 0; i < m; ++i)
        for (int j = 0; j < n; ++j)
            if (isalpha(board[i][j])) tiles[board[i][j] - 'A'].push_back({ j, i });
    ...
    for (int i = 0; i < 26; ++i) {
        ...
        while (!q.empty()) {
            ...
            if (board[y][x] != '.' && board[y][x] != startAlpha) continue;

            if (d.left == 0 && x < n - 1) {
                Direction nextD = { 0, 0, 0, 1, d.count };
                if (!d.right) ++nextD.count;
                q.push({ { x + 1, y }, nextD });
            }
            if (d.right == 0 && x > 0) {
                Direction nextD = { 0, 0, 1, 0, d.count };
                if (!d.left) ++nextD.count;
                q.push({ { x - 1, y }, nextD }); 
            }           
            if (d.bottom == 0 && y > 0) {
                Direction nextD = { 1, 0, 0, 0, d.count };
                if (!d.top) ++nextD.count;
                q.push({ { x, y - 1 }, nextD });
            }
            if (d.top == 0 && y < m - 1) {
                Direction nextD = { 0, 1, 0, 0, d.count };
                if (!d.bottom) ++nextD.count;
                q.push({ { x, y + 1 }, nextD });
            }
        }
    }
    ...
    bool isEmpty = true;
    for (int i = 0; i < 26; ++i)
        if (!tiles[i].empty()) isEmpty = false;

    return isEmpty ? answer : "IMPOSSIBLE";
}
